...
inline void clampAllParameters() {
  for (int i = 0; i < WAVE_COUNT; i++) {
    WaveformType wf = (WaveformType)i;

    if (waveformUsesRealDataTable(wf)) {
      
      waveformAmp[i] = clampf(waveformAmp[i], 0.0f, 1.0f);
      waveformFreq[i] = clampf(waveformFreq[i], 0.10f, 10.0f);
    } else {
...
inline const int16_t* realDataTableForWaveform(WaveformType wf) {
  switch (wf) {
    case WAVE_MITBIH_MLII:  return realMitBih100MliiCentered;
    case WAVE_EEG_OPEN:     return realEegEyesOpenCentered;
    case WAVE_EEG_CLOSED:   return realEegEyesClosedCentered;
    case WAVE_EMG_HEALTHY:  return realEmgHealthyCentered;
    case WAVE_FLATLINE_25V: return realFlatCentered;
    default:                return realFlatCentered;
  }
}

inline float realDataValueFromPhase(WaveformType wf, float phase) {
  if (phase < 0.0f) phase = phase - floorf(phase);
  if (phase >= 1.0f) phase = phase - floorf(phase);

  const int16_t* table = realDataTableForWaveform(wf);
  float tablePos = phase * (float)SIGNAL_TABLE_SIZE;
  int i0 = (int)tablePos;
  float frac = tablePos - (float)i0;

  if (i0 < 0) i0 = 0;
  if (i0 >= SIGNAL_TABLE_SIZE) i0 = 0;

  int i1 = i0 + 1;
  if (i1 >= SIGNAL_TABLE_SIZE) i1 = 0;

  float y0 = (float)table[i0];
  float y1 = (float)table[i1];
  float centered = y0 + (y1 - y0) * frac;

  return baseline + waveformAmp[(int)wf] * centered;
}

inline float realDataModel(WaveformType wf, float t) {
  float factor = waveformFreq[(int)wf];
  float phase = fmodf(t * factor, 1.0f);
  if (phase < 0.0f) phase += 1.0f;
  return realDataValueFromPhase(wf, phase);
}
...
inline float generateSignalForWaveform(WaveformType wf, float t) {
  switch (wf) {
    ...
    case WAVE_MITBIH_MLII:
    case WAVE_EEG_OPEN:
    case WAVE_EEG_CLOSED:
    case WAVE_EMG_HEALTHY:
    case WAVE_FLATLINE_25V:
      return realDataModel(wf, t);
...
inline float buildTableValueForIndex(int i) {
  ...
  switch (selectedWaveform) {
    ...
    case WAVE_EMG_HEALTHY:
    case WAVE_FLATLINE_25V:
      return realDataValueFromPhase(selectedWaveform, phase);
