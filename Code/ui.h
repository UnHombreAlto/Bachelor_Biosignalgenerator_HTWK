...
static ArrowButton btnMuxLeft  = {  20, 215, 40, 35, false };
static ArrowButton btnMuxRight = { 190, 215, 40, 35, true  };
...
inline const char* freqUnitToString(WaveformType wf) {
  if (waveformUsesPlaybackFactor(wf)) return "x";
...
inline void drawSignalPreview(WaveformType wf) {
  ...
  for (int i = 0; i < previewW - 8; i++) {
    ...
    else if (waveformUsesRealDataTable(wf)) scaleDenom = 2000.0f;
...
inline void drawMuxSelector() {
  gfx->fillRect(15, 208, 220, 48, black);

  drawArrowButton(btnMuxLeft, false);
  drawArrowButton(btnMuxRight, false);

  gfx->setTextColor(neonGreen);
  gfx->setTextSize(2);
  gfx->setCursor(72, 222);
  gfx->print("MUX ");
  if (selectedMuxChannel < 10) gfx->print("0");
  gfx->print(selectedMuxChannel);
}
...
inline void applyHoldAction(HoldAction action) {
  ...
  switch (action) {
    ...
    case HOLD_FREQ_MINUS:
      waveformFreq[i] -= waveformUsesPlaybackFactor(selectedWaveform) ? 0.10f : 1.0f;
      break;
    case HOLD_FREQ_PLUS:
      waveformFreq[i] += waveformUsesPlaybackFactor(selectedWaveform) ? 0.10f : 1.0f;
      break;
...
inline void handleTouchUi() {
  ...
  if (currentScreen == SCREEN_MAIN_MENU) {
    ...
    if (pointInRect(x, y, btnBack)) {
      ...
      animateButton(btnBack);
      drawWaveMenuPage();
    } else if (pointInArrow(x, y, btnMuxLeft)) {
      animateArrow(btnMuxLeft);
      changeMuxChannel(-1);
      drawMuxSelector();
    } else if (pointInArrow(x, y, btnMuxRight)) {
      animateArrow(btnMuxRight);
      changeMuxChannel(1);
      drawMuxSelector();
    }
  }
